...
#define f1 false // true - считываем с файла, false - рандомно заполняем тела
#define f2 false // true - запись в файл, false - НЕ ЗАПИСЫВАТЬ

#define sq(x) sqrt((x)) // если double, то sqrt. Если float, то sqrtf
...
__global__ void runge_kutta_it(Body* body_global, T2* body_global_temp, int N, T1 tau, int Nt) {

    T1 htau = tau / 2; //полушаг

    const int i = blockIdx.x * blockDim.x + threadIdx.x; //индекс текущего треда
    ...
    for (int m = 0; m < Nt; ++m) {
        ...
        for (int block = 0; block < N / block_size; ++block) {
            ...
            for (size_t j = 0; j < block_size; ++j)
            {
                ...
                T1 norma = sq(diff_r.x * diff_r.x + diff_r.y * diff_r.y + diff_r.z * diff_r.z);
                coeff = 1 / max1(norma, eps);
                coeff *= coeff * coeff;


                a_temp.x -= coeff * body_shared[j].w * diff_r.x;
                a_temp.y -= coeff * body_shared[j].w * diff_r.y;
                a_temp.z -= coeff * body_shared[j].w * diff_r.z;
            }
        ...
        for (int block = 0; block < N / block_size; ++block) {
            ...
            for (size_t j = 0; j < block_size; ++j)
            {
                ...
                coeff = 1 / max1(norma, eps);
                coeff *= coeff * coeff;
                a.x -= coeff * body_shared[j].w * diff_r.x;
                a.y -= coeff * body_shared[j].w * diff_r.y;
                a.z -= coeff * body_shared[j].w * diff_r.z;
            }
        ...
        body_global[i].r[0] += (body_global[i].v[0] + v_temp.x) * htau;
        body_global[i].r[1] += (body_global[i].v[1] + v_temp.y) * htau;
        body_global[i].r[2] += (body_global[i].v[2] + v_temp.z) * htau;

        body_global[i].v[0] += (a.x + a_temp.x) * htau;
        body_global[i].v[1] += (a.y + a_temp.y) * htau;
        body_global[i].v[2] += (a.z + a_temp.z) * htau;
    }
}
...
void runge_kutta(const int N, std::vector<Body> body_global, const int Nt, const double tau, const double tau_f) {
    ...
    cudaMalloc(&body_global_device, N * sizeof(Body)); //выделение памяти
    cudaMalloc(&body_temp_device, N * sizeof(T2)); //выделение памяти
    cudaMemcpy(body_global_device, body_global.data(), N * sizeof(Body), cudaMemcpyHostToDevice);  //копирование на видеокарту
    ...
    if (f2) {
        ...
        for (int m = n_f; m <= Nt; m += n_f) {


            runge_kutta_it << <blocks, BLOCK_SIZE >> > (body_global_device, body_temp_device, N, tau, n_f); //итерации ркнге кутта
            cudaDeviceSynchronize();

            cudaMemcpy(body_global.data(), body_global_device, N * sizeof(Body), cudaMemcpyDeviceToHost); // копирование с видеокарты на хост
    ...
    if (f2)
        ...
        runge_kutta_it << <blocks, BLOCK_SIZE >> > (body_global_device, body_temp_device, N, tau, Nt);

        cudaEventRecord(finish, 0);
        cudaEventSynchronize(finish);
        cudaEventElapsedTime(&time, start, finish);

        float dt; // working time meter
        cudaEventElapsedTime(&dt, start, finish);
        cudaEventDestroy(start);
        cudaEventDestroy(finish);

        std::cout << "Time:  " << (time / Nt) / 1000;
